...
version: 2026-07-23（新增 2-5「查無資料的表達方式（null / Optional / 例外）」——適用範圍擴及 Manager／Domain Service／靜態查表，含空集合語意一致性與業務／系統例外分型判準；來源：多次在 Review 後才被抓到回傳 null 與例外分型錯誤的實務案例）
...
### 2-5 查無資料的表達方式（null / Optional / 例外）

> **核心原則：不要讓呼叫端猜。** 方法簽名必須讓「這裡可能沒有東西」與「沒有東西代表什麼」都是型別系統或例外型別可見的事實，而不是靠讀實作、看註解、或試出 NPE 才知道。

#### 三種表達方式的選擇

| 情境 | 正確表達 | 理由 |
|------|---------|------|
| 查無資料屬**正常業務情境**（使用者可能沒設定、可選欄位、選填關聯） | `Optional<T>` | 呼叫端被強制顯式處理；`orElse` / `map` / `ifPresent` 讓後續分支自然成形 |
| 查無資料屬**系統異常**（設定檔缺漏、必要主檔不存在、服務啟動前置條件未滿足） | 拋例外 | 這不是「沒有」，是「壞了」。回 `Optional` 會讓呼叫端誤以為是可接受的空值而寫出防禦性 fallback，把故障掩蓋成正常流程 |
| 集合類查無 | 空集合（`List.of()` / `Set.of()`） | 空集合本身就是完整語意，包 `Optional<List<T>>` 只是多一層無意義的解包 |
| **任何情境** | ❌ `null` | 「查無資料」與「發生錯誤」混在同一種訊號裡，呼叫端無從區分也無從被提醒 |
...
```java
// ❌ 回 null：呼叫端只能靠讀實作才知道要判斷
public XxxSettingVO findSetting(String key) {
    return settingCache.get(key);          // 查無時 return null
}
// 呼叫端被迫寫成這樣，而且很容易忘記：
XxxSettingVO setting = manager.findSetting(key);
if (setting == null) { ... }               // 漏掉就是 NPE

// ✅ 查無屬正常 → Optional
public Optional<XxxSettingVO> findSetting(String key) {
    return Optional.ofNullable(settingCache.get(key));
}

// ✅ 查無屬異常 → 拋例外，不要用 Optional 讓上層猜該怎麼辦
public XxxSettingVO getRequiredSetting(String key) {
    return Optional.ofNullable(settingCache.get(key))
...
```java
// ❌ Manager 把 Optional 拆掉再往上丟 null，等於白做
public Map<String, Integer> getSettings(int id) {
    return repository.findById(id).map(Xxx::getSettings).orElse(null);
}
...
查到了資料但內容為空（空 Map、空 List、空字串），與完全查不到，**語意上往往是同一件事**，處置卻常被寫得不一致——一個放行、一個報錯。審查時檢查：
...
判斷標準：問自己「這兩種狀態，呼叫端有理由做不同處置嗎？」沒有 → 合併成同一道判斷；有 → 在程式碼中明確寫出兩者的差異理由。

#### 例外型別的選擇（連帶檢查）

決定「查無 → 拋例外」之後，還要選對例外型別。**業務例外與系統例外必須分開**，因為它們的回應格式、可觀測管道、告警策略都不同：

| 例外類型 | 語意 | 典型處置 |
|---------|------|---------|
| 業務例外（自訂，如 `XxxInvalidException`） | 使用者的請求不合法 | 上層 catch → 轉為可讀的失敗回應（2xx 或 4xx）；寫入業務失敗記錄 |
| 系統例外（`NoSuchElementException` / `IllegalStateException` 等） | 服務端資料或狀態不完整 | 不被業務 catch 攔截，冒至最外層 → 通用錯誤回應（5xx）；**不**混入業務失敗統計 |

**常見錯誤**：把服務端資料不完整包成業務例外，回傳「您的請求失敗」。後果有三——① 使用者看到的原因是錯的；② 這些失敗混進業務失敗統計，稀釋真實的失敗率；③ 故障訊號被降級成一行 log，監控上看起來一切正常。
...
| 新增技術棧 | 第一節對應類別表格補一列 |
| 調整品質原則 | 第二節對應子章節 |
| 調整 null / Optional / 例外分型判準 | 第二節 2-5 |
