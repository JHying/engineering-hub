...
5. 從 Step 1 已讀取的 `{{master_index}}` 萃取「系統規格基準」章節純文字（僅這一段），供 Step 5 Standards Subagent 使用——**只傳這段萃取文字，不把 master_index 檔案路徑交給 Standards Subagent 自行讀取**，避免它連帶讀到服務清單、跨服務通訊拓撲等與效能嚴重度判斷無關的專案脈絡
...
但效能瓶頸/資料原子性的嚴重度判斷確實需要規模事實（同一段迴圈查詢，10 筆與千萬筆資料的嚴重度不同）——這不代表 Standards Subagent 需要知道業務目的才能判斷規模。`{{review_guide}}` 第 3-1 節已明定：效能門檻不是逐 ticket 判斷，而是「對應專案 KB 的 MASTER_INDEX → 系統規格基準」這個**專案層級、與 ticket 無關**的標準值（現有 QPS/TPS、資料量現狀）；要知道哪個服務適用哪組數字，只需要「這個異動檔案屬於哪個 service」這個**結構性事實**（從檔案路徑/package 判斷，不是業務意圖），不需要知道這個 ticket 想達成什麼。`{{master_index}}` 已在 Step 1 由主線載入——但主線只萃取「系統規格基準」這一段純文字貼進 Standards Subagent 的 prompt，不把整份 master_index 檔案路徑交給它自行讀取（Read 工具會讀進整份檔案，連服務清單、跨服務通訊拓撲等業務脈絡都會一併看到，實測驗證過這會需要 subagent 自律排除、而非結構性隔離，故改為主線先萃取），這樣才不會為了取得規模事實而繞回需要知道業務脈絡的老路。
...
## 系統規格基準（專案層級、與本 ticket 無關的標準值）
{主線從 {{master_index}} 萃取的「系統規格基準」章節純文字——只貼這一段的內容，不附帶 master_index 其他章節（服務清單、跨服務通訊拓撲等），避免 Standards Subagent 讀到規模事實以外的專案脈絡}；**若該專案 MASTER_INDEX 尚未建立此章節** → 略過規模比對，效能瓶頸僅依 review_guide 3-2～3-6 的規則型判斷（N+1、非原子操作、鎖範圍等，這些違規本身不因規模而改變對錯），並在輸出中標注「⚠️ 專案 KB 尚無系統規格基準，嚴重度未依實際規模校準」

## 輸出
依下方格式輸出「品質問題」「效能瓶頸 / 資料原子性」「設計模式」三區塊（格式見下）。

## 限制
不得讀取 spec / impl 檔案，不得詢問或推測這個 ticket 的需求描述 / AC / 功能目標。不得自行用 Read 工具開啟 `{{master_index}}` 或任何 MASTER_INDEX.md 檔案——上方已提供判斷所需的完整內容，自行讀取整份檔案會連帶看到服務清單、跨服務通訊拓撲等與效能嚴重度判斷無關的專案脈絡。系統規格基準只能用來判斷效能瓶頸的**嚴重度**，不得用它調整品質原則（OOP / SOLID / DDD）與設計模式的判斷標準。
```
