# HTTP Controller 契約生成（明細）

適用：`@RestController` / `@Controller` + `@RequestMapping` 的 HTTP endpoint。
...
見 `references/http-contract-templates.md`（`_valid.groovy` 與 `_invalid.groovy` 完整範本）。
