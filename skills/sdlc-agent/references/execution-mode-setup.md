...
> 若選擇「PM+SA（模式 5）」，略過此步，起點固定為「需求企劃」、終點固定為「Spec 轉化」（`$end_stage`=2）。
...
- `$end_stage` 未設定時預設為 5（QA）
- `$start_stage` ≤ 1 且 `$end_stage` ≥ 1 → 列出「需求企劃」行，否則省略該行
- `$start_stage` ≤ 2 且 `$end_stage` ≥ 2 → 列出「Spec 轉化（含 ADR 溝通）」行，否則省略該行
- `$start_stage` ≤ 3 且 `$end_stage` ≥ 3 → 列出「Spec-Driven 實作（含 ADR 驗證）」行，否則省略該行
- `$start_stage` ≤ 4 且 `$end_stage` ≥ 4 → 列出「Code Review」行，否則省略該行
- `$end_stage` ≥ 5 → 列出「QA」行，否則省略該行
...
stage 數量少於 5 時取前 N 碼（例如模式 5 僅「需求企劃」「Spec 轉化」兩個 stage，建議預設為 `C A`）。
