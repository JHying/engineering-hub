...
請輸入要更新的內容（擇一）：
  ...
  5. 去識別化的架構決策（更新共用 ADRs → `common_KBs/ADRs/`，不可含識別資訊）
  6. Code Review 記錄（新增 review-history/ 條目，可含 ticket 單號或直接描述）
  7. 技術探討 / 研究筆記（更新 `common_KBs/tech-research/`，去識別化）
...
3. 確認內容是否已完全去識別化（若有疑慮，輸出標注後停止，等候使用者確認）
...
4. 更新 `{$KB_ROOT}/knowledge/common_KBs/tech-research/index.md` 的筆記清單
