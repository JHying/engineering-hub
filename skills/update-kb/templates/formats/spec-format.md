...
此文件定義 `knowledge/specs/<TICKET>.md` 的標準格式。

Spec 記錄「要做什麼」：功能目標、資料流、商業規則、驗收條件、特殊限制。
不記錄「怎麼做」——class 位置、package、config 變更屬於 impl 文件（`knowledge/specs/impls/<TICKET>.md`）。
...
AC 條件與 Gherkin 的概述摘要
逐條列出測試案例，使用 checkbox 格式：
- [ ] 條件一
- [ ] 條件二
...
```
ClassName
  → reads/writes: key / topic / collection
  → next ClassName
      returns: field, field
  → WsSession.send(...) / Kafka.send(topic)
```
...
（若有）API response 結構、WebSocket 訊息格式、grpc、kafka、欄位說明。
...
- **具體值要記錄**：Redis key 格式、Kafka topic 名稱、MongoDB collection 名稱要明確寫出
